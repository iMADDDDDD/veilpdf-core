...
pub unsafe extern "C" fn veil_free_buffer(buf: VeilBuffer) {
    ...
    if !buf.error.is_null() && buf.error_len > 0 {
        drop(unsafe { Box::from_raw(std::ptr::slice_from_raw_parts_mut(buf.error, buf.error_len)) });
    }
}
...
pub unsafe extern "C" fn veil_split(ptr: *const u8, len: usize) -> VeilBuffer {
    if ptr.is_null() { return VeilBuffer::error("null input pointer".into()); }
    if len == 0 { return VeilBuffer::error("empty input buffer".into()); }
    if len > MAX_INPUT_SIZE { return VeilBuffer::error("input exceeds maximum size (512 MB)".into()); }
